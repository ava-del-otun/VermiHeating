...
modeled on the exposed outer-area basis. With the insulated exposed diameter
\begin{equation}
OD_{\text{ext}} = OD + 2t_{\text{ins}},
\end{equation}
this gives
\begin{equation}
U_{\text{heater}}
=
\left(
\frac{OD_{\text{ext}}}{ID\,h_{\text{inside,heater}}}
+\frac{OD_{\text{ext}}}{2k_{\text{wall}}}\ln\!\left(\frac{OD}{ID}\right)
+\frac{OD_{\text{ext}}}{2k_{\text{ins}}}\ln\!\left(\frac{OD_{\text{ext}}}{OD}\right)
...
=
\left(
\frac{OD}{ID\,h_{\text{inside,aer}}}
+\frac{OD}{2k_{\text{wall}}}\ln\!\left(\frac{OD}{ID}\right)
...
This section documents the moisture-transfer model, the constrained
NSGA-II/Pareto heating-selection workflow, and the numerical procedure used by the heater-design
...
\subsection{Constrained NSGA-II Pareto Selection and Design-Point Reporting}

The MATLAB model still generates the physics by evaluating a discrete sweep of
pitch, voltage, flow, and geometry candidates. The current standalone Python
report layer no longer chooses the final reported heating design point by
direct lexicographic ranking of the full sweep. Instead, it applies
constrained NSGA-II to the already evaluated discrete candidate set and then
reports one point from the feasible Pareto set.

\subsubsection{Candidate Objectives and Constraint Residuals}

For each operating point, the code computes the full-power two-node bed
temperatures $T_b$ and $T_t$, their spread
\begin{equation}
\Delta T_{\mathrm{bed}} = |T_b - T_t|,
\end{equation}
the colder-node temperature
\begin{equation}
T_{\mathrm{cold}} = \min(T_b,T_t),
\end{equation}
...
The current heating-objective vector is
\begin{equation}
\mathbf{F}
=
\left[
...
\right],
\end{equation}
so the reported Pareto front minimizes total electrical power, maximizes the
colder-node bed temperature, and minimizes bottom--top spread.

The inequality-constraint residual vector is
\begin{equation}
\mathbf{G}
=
\left[
...
with
\begin{align}
G_1 &= \max(I_{\mathrm{tot}} - I_{\max}, 0), \\
G_2 &= \max(T_{\mathrm{wire}} - T_{\mathrm{wire,max}}, 0), \\
G_3 &= \max(T_{\mathrm{air,out}} - T_{\mathrm{air,max}}, 0), \\
G_4 &= \max(\Delta P - \Delta P_{\max}, 0), \\
G_5 &= \max(\dot{m}_{\mathrm{water,day}} - \dot{m}_{\mathrm{water,max}}, 0), \\
G_6 &= \max(u_{\mathrm{hole}} - u_{\mathrm{hole,max}}, 0), \\
G_7 &= \max(T_{b,\min} - T_b, 0), \\
G_8 &= \max(T_{t,\min} - T_t, 0), \\
G_9 &= \max(\Delta T_{\mathrm{bed}} - \Delta T_{\max}, 0), \\
G_{10} &= \max(\mathrm{dutyNeeded} - 1, 0), \\
G_{11} &= \max(-Q_{\mathrm{bed,total}}, 0).
\end{align}

Thus a feasible heating point is one for which every component of
$\mathbf{G}$ is zero. In particular, the reported design point must satisfy
the current, wire-temperature, outlet-air, pressure-drop, water-loss,
hole-velocity, bottom-temperature, top-temperature, spread, and full-load
heating requirements simultaneously.

\subsubsection{NSGA-II on the Discrete Candidate Grid}

The current implementation uses NSGA-II through \texttt{pymoo}, but it does
not replace the underlying physics solve. Instead, the algorithm operates on
integer-valued indices into the already evaluated discrete pitch, voltage, and
flow arrays. The present default population size in the JSON configuration is
\begin{equation}
N_{\mathrm{pop}} = 50.
\end{equation}

This choice preserves the rigor of the full MATLAB candidate evaluation while
replacing the old single-score final-selection step with a proper
multiobjective feasible-Pareto search.

\subsubsection{Reported Design Point from the Feasible Pareto Set}

NSGA-II returns a feasible Pareto candidate set in the objective space above.
The standalone summary and report then choose one displayed design point from
that feasible Pareto set by a user-configurable report-priority order. In the
current default configuration, the reported-point ordering is:
\begin{enumerate}
\item maximize the colder-node temperature $T_{\mathrm{cold}}$;
\item minimize total electrical power $P_{\mathrm{tot}}$;
\item minimize bottom--top spread $\Delta T_{\mathrm{bed}}$;
\item use mean bed temperature only as a later tie-breaker; and
\item use residual heat shortfall only as a final tie-breaker.
\end{enumerate}

Accordingly, the reported design point is \emph{not} an arbitrary weighted
sum. It is the first point in the feasible Pareto set after applying the
configured report-priority order.

\subsubsection{Fallback Reference Point}

If no fully feasible Pareto point exists, the code reports no recommended
heating design point. In that case, it retains a best-available reference
point only for diagnostic plots and fallback summaries. That fallback point is
still based on a lexicographic least-violation ranking of the full sweep, so
the user can see which constraint is blocking feasibility without falsely
presenting an infeasible point as a final recommendation.

\subsubsection{Where the Pareto Workflow Is Used}

The constrained-NSGA-II heating workflow now drives:
\begin{enumerate}
\item the reported recommended point for the active vessel configuration;
\item the configuration-specific recommended point shown in the vessel
      comparison table; and
\item the active-configuration Pareto-candidate table and Pareto-front plot.
\end{enumerate}

The report-only duty-reference value
\texttt{model\_overrides.sweep.targetDuty} is still retained, but it remains a
display aid for the minimum-flow duty line rather than a replacement for the
full-feasibility condition above.
...
In the standalone Python heating reference-point solve, the helical wire inside
the heater tube also contributes a separate reduced-order obstruction term.
Rather than folding that effect into an arbitrary extra minor-loss constant, the
code now evaluates a projected-area drag surrogate based on the local velocity
component normal to the helix. This is an intentional reduced-order mapping of
the handbook sections on tubes with wire ribbing and spiral-ribbed air heaters:
those references indicate that the ribbing contributes finite aerodynamic
resistance in addition to the smooth-tube Darcy wall-friction term. The active
JSON configuration now exposes this closure through
\texttt{coilPressureModel}, \texttt{coilDragCoefficient}, and
\texttt{coilLossMultiplier} under \texttt{model\_overrides.heaterTube}.
...
branching loss by a textbook \(2\)-\(K\) surrogate for one branch path
through the manifold body, optionally scaled by the calibration multiplier
\texttt{splitterBodyLossMultiplier} (with legacy alias
...
\end{equation}
For a single electrical group, the total pressure drop reported for a candidate
point is the sum of the shared distribution path, the branch-private
distribution path, the heater-tube friction plus helical-obstruction terms, the
heater-to-aeration expansion, and the aeration-branch pressure requirement.

If multiple electrical series-count groups coexist in parallel, however, the
branch flows are not physically constrained to remain equal once the heater-side
power, air temperature, density, and helical-obstruction losses differ between
groups. In that case the correct reduced-order network statement is not to sum
all group drops in series, but to solve one common branch-side pressure
difference together with total-flow continuity. The common upstream header path
still contributes equally to every branch; the branch-private splitter,
connector, heater, expansion, and aeration losses are then evaluated at the
redistributed group-specific branch flows until those branch-path drops match.
The reported winter total therefore becomes
\[
\Delta P_{\mathrm{tot,heat}} = \Delta P_{\mathrm{dist,shared}} + \Delta P_{\mathrm{branch,common}},
\]
with
\[
\Delta P_{\mathrm{branch},1}(\dot m_1)=\Delta P_{\mathrm{branch},2}(\dot m_2)=\cdots=\Delta P_{\mathrm{branch,common}},
\qquad
\sum_g N_g\dot m_g = \dot m_{\mathrm{tot}}.
\]
If \texttt{headerEnabled = false}, then the shared upstream contribution
\(\Delta P_{\mathrm{dist,shared}}\) vanishes and the modeled distribution loss is
entirely branch-private; that is the current active JSON default. Under the
current default topology, where all heated branches belong to the same
electrical group, this redistributed-flow statement collapses to the same
single-group result as the simpler equal-branch-flow treatment.
...
standalone Python reporting logic. In the current layout, the main study and
output controls appear first, the heater-side export inputs are grouped under
\texttt{model\_overrides} together with the heating \texttt{optimization}
block, and the cooler-only inputs are grouped later under
\texttt{cooling\_mode}.
...
\item \texttt{model\_overrides.limits}, which defines explicit limits such as
      ...
      drop, and water loss;
\item \texttt{model\_overrides.optimization}, which defines the heating-side
      thermal feasibility thresholds such as maximum bottom--top spread and
...
\item \texttt{model\_overrides.aeration}, which includes the effective
      ...
      \texttt{branchConnectorLength\_m},
      \texttt{headerLossMultiplier},
      \texttt{splitterBodyLossMultiplier}, and
...
manifold loss is represented by the textbook tee-plus-valve \(2\)-\(K\)
surrogate, scaled if desired by \texttt{splitterBodyLossMultiplier}.
...
controls the standalone Python NSGA-II / Pareto heating-selection workflow
and the reported-design priority order, while the \texttt{radial\_profile} block controls the standalone worm-safe-distance
...
\begin{itemize}
\item \texttt{method}, \texttt{population}, \texttt{generations}, and
      \texttt{seed} define the standalone heating NSGA-II settings;
\item \texttt{objectives} defines the heating objective vector used to build
      the feasible Pareto set;
\item \texttt{report\_priority\_order} defines how one displayed heating
      design point is selected from that feasible Pareto set;
\item \texttt{priority\_order} is retained for best-available fallback
      ranking and diagnostics when no feasible Pareto point exists;
...
For each selected spell $j$, the model now samples a spell-specific anomaly
from a bounded normal law:
\begin{equation}
\Delta T_{h,m,j}
\sim
\mathcal{N}_{[\Delta T_{h,m}^{\mathrm{low}},\,\Delta T_{h,m}^{\mathrm{high}}]}
\!\left(\mu=\Delta T_{h,m},\ \sigma=\sigma_{\Delta T,h,m}\right),
\end{equation}
where the monthly low/high bounds and spread are configured in
\texttt{year\_round\_analysis.climate\_data.heat\_wave.*}. The implementation
uses a fixed RNG seed (\texttt{spellAnomalyRngSeed}) for reproducibility. In
this repository state, these monthly spell bounds are NOAA-derived from
\texttt{USW00014740} daily-summaries (2016--2025) and loaded from
\texttt{Heat Transfer Study/data/climate/noaa\_heatwave\_spell\_anomaly\_bounds\_USW00014740\_2016\_2025.json}
generated by
\texttt{Heat Transfer Study/tools/derive\_noaa\_heatwave\_spell\_bounds.py}.
...
\section{Model Credibility, Scope, and Sign-Off Requirements}

\subsection{Physics Blocks with the Strongest Current Basis}

For the present design workflow, the following blocks have the strongest
engineering basis and are appropriate for comparative design decisions:
\begin{enumerate}
\item temperature-dependent Nichrome electrical resistance and resistive heat
generation in the wire;
\item series--parallel electrical loading of identical heater branches;
\item forced convection and friction-factor-based pressure drop in the heater
and aeration tubes using established internal-flow correlations;
\item heater-shell loss through the tube wall and insulation by a standard
series-resistance formulation;
\item perforation continuity, hole exit velocity, and the bookkeeping of
useful bed heating as
\begin{equation}
Q_{\mathrm{to\,bed}}
=
\sum \left(q_{\mathrm{wall}} + q_{\mathrm{jet}} - q_{\mathrm{evap}}\right);
\end{equation}
\item natural-convection vessel loss to greenhouse air using established
vertical- and horizontal-surface correlations; and
\item year-round re-solving of the already-selected heating and cooling
reference points under a representative climate driver rather than assuming
fixed design-point power throughout the year.
\end{enumerate}

These blocks should be regarded as the most reliable parts of the current
model for ranking candidate voltages, flows, wire diameters, vessel-top
configurations, and electrical topologies.

\subsection{Reduced-Order or Screening-Level Blocks}

Other blocks are still physically motivated, but they remain reduced-order or
screening approximations:
\begin{enumerate}
\item the two-node bed model, which resolves only a bottom and top substrate
temperature rather than a full spatial temperature field;
\item the covered-top-with-small-vent extension, which treats the vent as an
area-limited top opening and does not explicitly resolve buoyancy-driven
exchange through the vent jet;
\item the splitter/manifold network model, for which the unresolved internal
losses are represented by a lumped splitter coefficient;
\item the evaporation model, which is internally consistent and source-backed
but still depends on a simplified wetted-area estimate and a Lewis-relation
mass-transfer closure;
\item the radial warm-air plume and worm-habitat exclusion calculation, which
is a post-processing estimate rather than a resolved flow field; and
\item the summer spot-cooler / assist-blower representation, which is
appropriate for feasibility screening but still relies on a simplified fan
curve, simplified spot-cooler performance mapping, and a reduced psychrometric
closure.
\end{enumerate}

Accordingly, the present model is best interpreted as a design and
optimization tool rather than a certification-grade hardware prediction.

\subsection{What the Current Model Is Appropriate For}

In its present form, the model is appropriate for:
\begin{enumerate}
\item comparing candidate heater geometries, flow rates, wire diameters, and
electrical topologies;
\item identifying hard-constraint-safe operating regions for current, wire
temperature, outlet-air temperature, pressure drop, water loss, and bed
temperature thresholds;
\item ranking vessel-top configurations such as uncovered, covered, and
covered with a small vent hole;
\item estimating seasonal electricity, water, and cost trends; and
\item selecting the most defensible prototype configurations for laboratory or
greenhouse testing.
\end{enumerate}

It should not, by itself, be treated as sufficient evidence for final
hardware release, guaranteed worm-safe local exposure, or formal thermal
safety sign-off under all faulted operating conditions.

\subsection{Requirements for Final Hardware Sign-Off}

A final hardware sign-off model would require all of the following:
\begin{enumerate}
\item a frozen as-built geometry, including the actual heater wire route,
support details, electrical joints, clearances, manifold geometry, vent
geometry, and enclosure details;
\item measured electrical and thermal properties for the purchased hardware,
including the actual wire resistance--temperature relation, insulation
performance, contact resistances, and shell temperatures;
\item measured hydraulic data for the real distribution hardware, especially
the blower curve, splitter/manifold loss coefficients, connector losses, and
exit losses, rather than placeholder or lumped estimates;
\item transient validation tests at representative winter and summer boundary
conditions, including startup, steady operation, duty-cycled operation, and
shutdown;
\item explicit fault-case evaluation, including blocked or reduced airflow,
blower failure, vent blockage, relay failure, and sensor failure;
\item uncertainty quantification showing that the validated model error band is
small enough that all limits are still met with margin after uncertainty is
applied; and
\item a higher-fidelity substrate model capable of resolving local hot spots,
temperature gradients, and moisture migration in the bed.
\end{enumerate}

Only after those requirements are met should the model be treated as a final
hardware sign-off basis rather than as a prototype-selection tool.

\subsection{What a 2D/3D Conjugate Porous/Moist Model Would Entail}

For this project, a sign-off-grade higher-fidelity model would be a
\emph{conjugate} model because it would solve the coupled physics in the
solid hardware, the flowing air, and the porous moist substrate
simultaneously rather than treating the bed as only two lumped temperature
nodes.

At minimum, such a model would include:
\begin{enumerate}
\item conduction in the metal and insulated hardware domains, including the
heater tube, aeration tubes, vessel walls, and cover;
\item fluid flow in the internal air passages and, if resolved, through the
perforations and the nearby free-plume region;
\item porous-medium flow and heat transfer in the substrate, with air exchange
between the perforations and the bed;
\item transient energy storage in the substrate and walls;
\item moisture transport in the substrate, including evaporation,
humidification of the gas phase, and the associated latent heat sink; and
\item realistic boundary conditions for greenhouse air, cover openings, and
all forced-flow inlets and outlets.
\end{enumerate}

In practical terms, this means solving coupled conservation equations for:
\begin{enumerate}
\item mass and momentum in the open-flow regions;
\item energy in the wire, tube walls, insulation, air, and porous substrate;
\item species transport for water vapor in the gas phase; and
\item liquid-moisture storage and migration in the porous substrate, either by
a saturation-based porous-media model or by an experimentally calibrated
effective-moisture formulation.
\end{enumerate}

Relative to the current model, the main benefit would be the ability to
predict spatially resolved quantities such as:
\begin{enumerate}
\item local worm-exposure temperatures near perforations and near the tube
surface;
\item local bed drying or wetting zones;
\item nonuniform temperature fields caused by tube placement, wall proximity,
and vent placement; and
\item transient recovery and overshoot during startup, shutdown, and disturbed
airflow events.
\end{enumerate}

The main cost would be substantially higher model complexity, substantially
greater computational time, and a much larger experimental burden for
calibration and validation. For that reason, such a model is usually reserved
for final design verification after the reduced-order model has already
narrowed the design space to one or two promising hardware configurations.
