...
- `model_overrides.bin`: active vessel geometry/top condition for the MATLAB export; the current default is the closed-top vented case, the widened-bin override now lives here as `width_m`, and the standalone Python bin-loss model now reads `bottomBoundaryMode` to choose the insulated-bottom ambient boundary closure
...
- `model_overrides.heaterTube`: editable coil-geometry overrides such as `coilMeanD_m`, `coilSpan_m`, and the standalone Python heater-side pressure-loss controls `coilPressureModel`, `coilDragCoefficient`, and `coilLossMultiplier`
...
- In the standalone Python bin-loss model, insulated side walls, the covered top, and the elevated bottom now use a short outer-skin fixed-point solve before evaluating the external natural-convection coefficient, so `T_s` in those wall-side Churchill-Chu / horizontal-plate closures is the exterior insulation-surface temperature rather than the bed bulk temperature.
- `model_overrides.bin.bottomBoundaryMode = "elevated_air_gap"` is now the default reduced-order choice for the insulated bottom. In that mode the underside uses the downward-facing horizontal-plate stability branch; any other value falls back to the legacy side-wall surrogate.
...
- In the standalone Python heating reference-point solve, the heater-tube pressure drop now includes a separate reduced-order helical-coil obstruction term. The default model is `coilPressureModel = "projected_area_drag"`, which uses the wire projected area per axial length, the local normal-velocity component to the helix, and the calibration-ready settings `coilDragCoefficient` and `coilLossMultiplier` under `model_overrides.heaterTube`.
...
- In the standalone Python heating reference-point solve, those terms are split into a shared upstream contribution (`header` plus `header->splitter`) and a branch-private contribution (`splitter body`, `connector friction`, `connector->branch`). If multiple electrical series-count groups coexist in parallel, the code now solves a common branch-path pressure drop together with total-flow continuity instead of forcing equal branch flow and then taking only a post hoc maximum across groups.
...
- The current hole-scale moisture closure combines Idel'chik's orifice-discharge relation
  `m_dot_rel = C_d A_h sqrt(2 rho Delta p)` with a direct Frössling mass-transfer surrogate
  `Sh = M_Sh (2 + 0.552 Re^0.5 Sc^(1/3))`, evaluated on the local perforation discharge velocity.
- The main aeration-tube evaporation model no longer reuses Churchill-Bernstein plus a Lewis conversion.
  It now forms `h_m` directly from `Sh`, `D_va`, and `d_h`, with `Sc = 0.60` for air/water-vapor service
  and the released-air vapor density taken from the local tube-side humidity ratio.
...
- The plume energy balance is re-solved locally as
  ...
  `q'_lat = m'_evap h_fg`.
- The radial plume post-processing still uses its own local Lewis-relation mass-transfer surrogate; it is not the same closure as the hole-scale Frössling model used in the core aeration-tube moisture calculation.
